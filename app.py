...
# Initializing RECCOMEND_DATA utility
# class found in /lib
REC_DATA = RECCOMEND_DATA()

# Initializing Flask app with
# static_url_path set to empty string default is 'static'
...
def send_image(path):
    # offer up the image files files for consumption
...
def make_recommendation():
    ...
    # [{i: json_dict[i]} for i in json_dict if i != 'campaign_objective']
    slice_parameters = json_dict

    # set default objects if none given
    objectives = json_dict.get(
        'campaign_objective', look_up_table.LOOK_UP_TABLE['campaign_objective'])
    ...
    for game_id in uniq_games:
        ...
        gm_kys_view[game_id]['click_through_rate'] = REC_DATA.calculates_rates(
            gm_kys_view[game_id]['click-through-event'], gm_kys_view[game_id]['impression'])

        gm_kys_view[game_id]['engagement_rate'] = REC_DATA.calculates_rates(
            gm_kys_view[game_id]['first_dropped'], gm_kys_view[game_id]['impression'])
    ...
    ind_sort = np.argsort([gm_kys_view[game_id]['rec_scores']
                           for game_id in uniq_games])[::-1]
